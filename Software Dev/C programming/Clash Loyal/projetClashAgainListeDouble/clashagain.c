    ...
    for(int i =0; i <LARGEURJEU; i++){
        for(int j=0; j<HAUTEURJEU; j++){
            if(!estvide_zone(jeuEncours[i][j])&& copy[i][j] == NULL){
                printf("test conversion : ajout unite surplateau  i: %d j: %d \n", i,j);
...
int AcheteUnite(TListePlayer *player, int elixir){
    ...
    //AjouterUnite(*player, creeChevalier(7,5));
...
TListePlayer PlayerInZone(Tunite unite, TplateauJeuUpgrade JeuUpgrade){
    ...
    printf("On display allplayer apres le concat \n");
    displayListPlayer(finallist);
    return finallist;
}
...
void displayListPlayer(TListePlayer player){
    while(player != NULL){
        printf("%d %d vitesse : %f, points de vie %d : ", player->pdata->posX, player->pdata->posY, player->pdata->vitesseAttaque, player -> pdata -> pointsDeVie);
        printf("\n");
        player=player->suiv;
    }
}
TListePlayer test_function(){
    T_listedouble zone1 = creerTzoneVide();
    TListePlayer test,test2,test3,test4;
    initListe(&test);initListe(&test2),initListe(&test3),initListe(&test4);

    for(int i = 0; i<15; i++){
        zone1 = ajouterUnitedansZone(zone1, creeDragon(8,3,rouge));
        zone1 = ajouterUnitedansZone(zone1, creeArcher(8,2,bleu));
    }
    /*test = getAllUnitInZone(&zone1);
    test2 = getAllUnitInZone(&zone1);
    test3 = getAllUnitInZone(&zone1);*/
    test4 = getAllUnitInZone(&zone1);

    /*tri_selection_liste(test,comp_couleur_rouge);
    tri_selection_liste(test2,comp_vitesse_asupb);
    tri_selection_liste(test3,comp_vie_ainfb);

    while (test !=NULL){
    printf(" couleur 0 = bleu, 1 = rouge. La couleur est de %d \n", test->pdata->couleur);
    test = test-> suiv;
    }

    while (test2 !=NULL){
    printf(" couleur 0 = bleu, 1 = rouge. La vitesse est de %f \n", test2->pdata->vitesseAttaque);
    test2 = test2-> suiv;
    }
    while (test3 !=NULL){
    printf(" couleur 0 = bleu, 1 = rouge. La point de Vie est de %d \n", test3->pdata->pointsDeVie);
    test3 = test3-> suiv;
    }*/
    return test4;
}
...
void MinusLife(Tunite *player1, Tunite *player2){
    if(player1->pointsDeVie < player2 ->degats){
        player1 -> pointsDeVie = 0;
    }else{
        player1->pointsDeVie = player1->pointsDeVie - player2->degats;
        }
...
void movep2(TListePlayer player, TListePlayer player2, TplateauJeu jeu)
{
    ...
    while(current != NULL)
    {
        int x = current -> pdata -> posX ;
        int y = current -> pdata -> posY ;

        if((current -> pdata -> nom) == 1 || (current -> pdata -> nom) == 0){
            current = current -> suiv;
        }else{
            if(isunity(player2,x+1,y) || isunity(player2,x-1,y) || isunity(player2,x,y+1)|| isunity(player2,x,y-1)){
                current = current -> suiv;
